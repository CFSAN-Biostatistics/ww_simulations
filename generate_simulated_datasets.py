...
def get_results(in_file, primer_amplicon, id_file, number_reads):
    ...
    neb_formatted.to_csv(primer_amplicon.split(".")[0] + "_for_in_silico_pcr.tsv", index = None, header = False, sep = "\t")
    ...
    for ref_id, proportion in ref_ids.items():
        bed_info[ref_id] = []
        with open(ref_id + "_" + primer_amplicon.split(".")[0] + ".bed", "r") as bed_file:
    ...
    for ref_id, proportion in ref_ids.items():
        ...
        subprocess.call(["art_illumina", "-amp", "-p", "-sam", "-na", "-i", input_id, "-l", "250", "-f", "1",  "-o", output_id])
...
def main(args):
    ...
    get_results(in_file, primer_amplicon, id_file, number_reads)
