...
results_directories = [ "./art_L75_F150_ARTICv4/", "./art_L75_F150_nebvss1a/", "./art_L75_F150_QIAseq/",
"./art_L150_F300_QIAseq/", "./art_L150_F300_ARTICv4/", "./art_L150_F300_nebvss1a/"]
...
for results_directory in results_directories: 
    ...
    for res_dir in dirs:
        ...
        for idx, f in enumerate(pos_info_files):
            rep = f.split("/")[2]
            print(rep)
            res_df = pd.read_csv(f, header = None, sep = "\t", names = ["Position", "Coverage", "Quality"])
            res_df["Rep"] = rep
    ...
    all_res_df.to_csv(results_directory.replace("./", "").replace("/", "") + "_coverage_results.tsv", sep = "\t", index = None, na_rep = "UNK")
