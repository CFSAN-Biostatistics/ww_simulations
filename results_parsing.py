...
dirs = [i for i in os.listdir(results_directory) if "_results" in i and "primer" not in i]
        ...
        for line in freyja_res:
            if "summarized" in line:
                ...
                for i in range(0, num_vars - 1):
                    var_info = to_parse[i].replace("[('", "").replace(", ('", "").replace("'", "")
...
for idx, f in enumerate(allCovid_bracken_files):
    ...
    try:
        ...
        pass
allCovid_bracken_df = pd.merge(allCovid_bracken_df, rep_info_df, on = ["who", "Rep"], how = "left")
...
for idx, f in enumerate(majorCovid_bracken_files):
    ...
    try:
        ...
        pass
majorCovid_bracken_df = pd.merge(majorCovid_bracken_df, rep_info_df, on = ["who", "Rep"], how = "left")
...
for idx, f in enumerate(kallisto_abundance_files):
    ...
    try:
        ...
        res_df_new = res_df.pivot_table(index='target_id_simplified', values='tpm', aggfunc='sum')
        ...
        pass
kallisto_abundance_df = pd.merge(kallisto_abundance_df, rep_info_df, on = ["who", "Rep"], how = "left")
...
for res_dir in dirs:
    ...
    for f in contents:
        if "linearDeconvolution_abundance" in f:
            linearDeconvolution_abundance_files.append(results_directory + res_dir + "/" + f)

linearDeconvolution_abundance_df = pd.DataFrame()
linear_deconvolution_mapping = pd.read_csv("lineardeconvolution_who_mapping.txt", sep = "\t", header = 0)
linear_deconvolution_mapping["who"] = linear_deconvolution_mapping["who"].str.upper()
linear_deconvolution_mapping["LinearDeconvolution"] = linear_deconvolution_mapping["LinearDeconvolution"].str.upper()
for idx, f in enumerate(linearDeconvolution_abundance_files):
    ...
    try:
        ...
        res_df = pd.merge(res_df, linear_deconvolution_mapping, on = "LinearDeconvolution", how = "left")
        ...
        linearDeconvolution_abundance_df = linearDeconvolution_abundance_df.append(res_df_new)
    except:
        pass
linearDeconvolution_abundance_df = pd.merge(linearDeconvolution_abundance_df, rep_info_df, on = ["who", "Rep"], how = "left")
...
df = pd.concat([linearDeconvolution_abundance_df, allCovid_bracken_df, majorCovid_bracken_df, kallisto_abundance_df, freyja_res_df], 
              ignore_index=True)
...
df.to_csv(platform + "_L" + read_length + "_F" + fragment_length + "_" + amplicon_panel + "_results.tsv", 
          sep = "\t", index = None, na_rep = "UNK")
